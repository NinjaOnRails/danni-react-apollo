...
const uploadCusThumbnail = (file, id, { signature, timestamp }, youtubeId) => {
  // Cloudinary API direct upload endpoint

  // Prepare params to send
  const publicId = `thumbnail-${id}-${youtubeId}`;
  const tags = `${id},${youtubeId},${timestamp},custom-thumbnail,custom,thumbnail,audio-thumbnail,picture,image`;
...
export { uploadAudio, uploadAvatar, uploadCusThumbnail };
